...
findPackageDependencyOrder(fs, ".", (function(packageFiles) {
console.dir(packageFiles, { depth: null });
}), (function(error) {
return panic(error);
}));
...
var JsonPackage = {
jsonPackage: function(account, repository, version, dependencies) {
return {_: "jsonPackage", account: account, repository: repository, version: version, dependencies: dependencies};
}
};

var JsonPackageDependency = {
jsonPackageDependency: function(module, import_, account, repository, version) {
return {_: "jsonPackageDependency", module: module, import_: import_, account: account, repository: repository, version: version};
}
};

var Package = {
package_: function(account, repository, version, dependencies) {
return {_: "package", account: account, repository: repository, version: version, dependencies: dependencies};
}
};

var PackageDependency = {
packageDependency: function(module, import_, account, repository, version) {
return {_: "packageDependency", module: module, import_: import_, account: account, repository: repository, version: version};
}
};

var PackageFiles = {
packageFiles: function(package_, path, files) {
return {_: "packageFiles", package_: package_, path: path, files: files};
}
};

var PackageResolver = {
packageResolver: function(fileSystem, through, packages, order) {
return {_: "packageResolver", fileSystem: fileSystem, through: through, packages: packages, order: order};
}
};
...
function jsonToJsonPackage(jsonText) {
return JSON.parse(jsonText);
}

function jsonDependenciesToDependencies(jsonDependencies) {
return map(jsonDependencies, (function(json) {
return PackageDependency.packageDependency(jsonOption(json.module), (function(_match) { switch(_match._) {
case "some": return (function(){
var import_ = _match.value;
return import_;
})();
case "none": return (function(){
return [];
})();
}})(jsonOption(json.import_)), jsonOption(json.account), orPanic(jsonOption(json.repository)), jsonOption(json.version));
}));
}

function jsonPackageToPackage(json) {
return Package.package_(jsonOption(json.account), orPanic(jsonOption(json.repository)), jsonOption(json.version), (function(_match) { switch(_match._) {
case "some": return (function(){
var jsonDependencies = _match.value;
return jsonDependenciesToDependencies(jsonDependencies);
})();
case "none": return (function(){
return [];
})();
}})(jsonOption(json.dependencies)));
}

function doFindPackageFiles(fs, package_, path, onSuccess, onError) {
var directory = (path + "/lim");
var moduleFiles = newArrayBuilder();
return readDirectory(fs, directory, (function(files) {
when(files.length == 0, (function() {
return onSuccess(PackageFiles.packageFiles(package_, path, []));
}));
return each(files, (function(file) {
var filename = (path + "/lim/" + file);
return readTextFile(fs, filename, (function(text) {
moduleFiles.push(Pair.pair(filename, text));
return when(moduleFiles.size() == files.length, (function() {
var sortedFiles = sortByString(moduleFiles.drain(), (function(p) {
...
return onSuccess(PackageFiles.packageFiles(package_, path, sortedFiles));
}));
}), onError);
}));
}), onError);
...
when(depth > order, (function() {
return resolver.order.set(name, depth);
}));
...
function findPackageDependencyOrder(fs, path, onSuccess, onError) {
var resolver = PackageResolver.packageResolver(fs, newStringMapBuilder([]), newStringMapBuilder([]), newStringMapBuilder([]));
return doFindPackageDependencyOrder(resolver, 1, path, (function() {
var order = sortByInt(resolver.order.toArray(), (function(o) {
return -o.second;
}));
var packages = map(order, (function(o) {
return resolver.packages.invoke(o.first);
}));
